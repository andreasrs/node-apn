...
Connection.prototype.connect = function () {
	...
	this.initialize().then(function () {
		...
		// Pass in securityOptions. Disable SSL by default (POODLE)
		var disableSSL = require('constants').SSL_OP_NO_SSLv3 | require('constants').SSL_OP_NO_SSLv2;
		socketOptions.securityOptions = this.options.securityOptions || disableSSL;
...
Connection.prototype.createConnection = function() {
	this.connect().then(function () {
		...
		var self = this;
		sock.socketId = this.socketId++;
		sock.currentId = 0;
		sock.cachedNotifications = [];
		this.deferredConnection = null;
		this.sockets.push(sock);
		if (this.options.enhanced && this.options['ackTimeout']){
			// if ackTimeout is set, we will send a known bad message after this set amount of time to
			// implicitly acknowledge all messages that were sent beforehand.
			var ackTimeout = setTimeout(function(){
				if ( sock.cachedNotifications.length ) {
					self.transmitNotification(sock,{ "notification": new Notification(), "recipient": new Device('aaaa1111'), "retryLimit": 0 });
				}
	...
	if (data[0] == 8) {
		...
		if (foundNotification) {
			while (temporaryCache.length) {
				var acknowledgedNotification = temporaryCache.shift();
				// We are discarding this message because we know implicitly
				// (by virtue of apple throwing an error on a subsequent notification)
				// that apple has acknowledged it was sent without errors
				if(this.options.enhanced) {
					this.emit('acknowledged', acknowledgedNotification.notification, acknowledgedNotification.recipient);
				}
			}
